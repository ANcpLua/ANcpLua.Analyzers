...
/// <summary>
///     Tests for AL0080: Missing resilience configuration analyzer.
/// </summary>
public sealed partial class Al0080MissingResilienceConfigurationTests {
    ...
    private static Task VerifyAsync(string source) {
        var test = new CSharpAnalyzerTest<Al0080MissingResilienceConfigurationAnalyzer, DefaultVerifier> {
    ...
    #region AL0080: Should Report

    [Fact]
    public Task AL0080_ShouldReport_AddHttpClient_NoResilience() =>
        VerifyAsync($$"""
                      public class Startup {
                          public void ConfigureServices({{Isc}} services) {
                              {|AL0080:services.AddHttpClient("MyClient")|};
                          }
                      }
                      """);

    [Fact]
    public Task AL0080_ShouldReport_AddHttpClient_Generic_NoResilience() =>
        VerifyAsync($$"""
                      public interface IMyClient { }
                      public class MyClient : IMyClient { }
                      public class Startup {
                          public void ConfigureServices({{Isc}} services) {
                              {|AL0080:services.AddHttpClient<IMyClient, MyClient>()|};
                          }
                      }
                      """);
    ...
    #region AL0080: Should Not Report

    [Fact]
    public Task AL0080_ShouldNotReport_WithStandardResilienceHandler() =>
        VerifyAsync($$"""
                      public class Startup {
                          public void ConfigureServices({{Isc}} services) {
    ...
    [Fact]
    public Task AL0080_ShouldNotReport_WithResilienceHandler() =>
        VerifyAsync($$"""
                      public class Startup {
                          public void ConfigureServices({{Isc}} services) {
                              services.AddHttpClient("MyClient")
    ...
    [Fact]
    public Task AL0080_ShouldNotReport_WithTransientHttpErrorPolicy() =>
        VerifyAsync($$"""
                      public class Startup {
                          public void ConfigureServices({{Isc}} services) {
                              services.AddHttpClient("MyClient")
    ...
    [Fact]
    public Task AL0080_ShouldNotReport_WithPolicyHandler() =>
        VerifyAsync($$"""
                      public class Startup {
                          public void ConfigureServices({{Isc}} services) {
                              services.AddHttpClient("MyClient")
    ...
    [Fact]
    public Task AL0080_ShouldNotReport_WithStandardHedgingHandler() =>
        VerifyAsync($$"""
                      public class Startup {
                          public void ConfigureServices({{Isc}} services) {
                              services.AddHttpClient("MyClient")
