...
/// <summary>
///     Tests for AL1313: forwarding available CancellationToken values to overloads that accept them.
/// </summary>
public sealed partial class Al1313CancellationTokenPropagationTests
    : AnalyzerTest<Al1313CancellationTokenPropagationAnalyzer> {
        ...
        VerifyAsync($$"""
                      ...
                      public sealed class C {
                          ...
                          public Task M() {
                              ...
                              return {|AL1313:_service.FetchAsync(42)|};
                          }
                      }
                      """);
...
/// <summary>
///     Code fix tests for AL1313: inserts the nearest CancellationToken argument.
/// </summary>
public sealed partial class Al1313CancellationTokenPropagationCodeFixTests
    : CodeFixTest<Al1313CancellationTokenPropagationAnalyzer, Al1313CancellationTokenPropagationCodeFixProvider> {
    private const string CommonStubs = """
                                      using System;
                                      using System.Threading;
                                      using System.Threading.Tasks;
    ...
    public Task ShouldInsertCancellationTokenInTheMiddle() =>
        VerifyAsync($$"""
                      ...
                      public sealed class C {
                          ...
                          public Task M() {
                              CancellationToken token = default;
                              return {|AL1313:_service.SaveAsync(42, "demo")|};
                          }
                      }
                      """,
            $$"""
        ...
        VerifyAsync($$"""
                      ...
                      public sealed class C {
                          ...
                          public async Task First(CancellationToken cancellationToken) {
                              await {|AL1313:_service.FetchAsync(1)|};
                              await {|AL1313:_service.FetchAsync(2)|};
                          }

                          public async Task Second(CancellationToken ct) {
                              await {|AL1313:_service.FetchAsync(3)|};
                          }
                      }
                      """,
            $$"""
